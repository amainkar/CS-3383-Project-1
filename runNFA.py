...
def main():
    ...
    if computation_strings == None:                                                 #if beta is empty call empty_beta()
        empty_beta(start_state, transition_dictionary, end_states)
    else:
...
def NFA_process(input_str, state,transition_dictionary, end_states):
    ...
    for next_state in transition_dictionary[state][current_char]:               #assuming our next state is in our dictionary 
        valid_tree = valid_tree or NFA_process(input_str[1:],next_state, transition_dictionary, end_states)        #then we recurse until we reach the end of our input string
    return valid_tree

##this function is used in the case that our beta is empty
def empty_beta(start_state, transition_dictionary, end_states):
    while True:
        input_str = input("Please input a string:")                             #user input
        if input_str == "":
            print("Bye bye.")
            break
        if (NFA_process(input_str,start_state, transition_dictionary, end_states)):                 
            print("Accepted.")
        else:
            print("Rejected.")
